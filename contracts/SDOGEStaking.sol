...
contract SDOGEStaking is Ownable {
    ...
    function stakeWithPermit(
        ...
        IERC2612Permit(sdoge).permit(msg.sender, address(this), amount_, expiry, v, r, s);
    ...
    /**
        @notice stake sdoge to enter escrow
        @param amount_ uint
    ...
    function _stake(uint256 amount_, address recipient_) internal returns (uint256) {
        ...
        IERC20(sdoge).safeTransferFrom(msg.sender, address(this), amount_);
        ...
        if (escrowPeriod > 0) {
            stakeInfo[recipient_] = Claim({
                ...
                gons: info.gons.add(ISSDOGE(sSDOGE).gonsForBalance(amount_)),
                expiry: expiryEpoch,
                lock: false
            });

            IERC20(sSDOGE).safeTransfer(escrowContract, amount_);
        } else {
    ...
    /**
        @notice retrieve sSDOGE from escrow
        @param recipient_ address
     */
    function claim(address recipient_) external lock {
        Claim memory info = stakeInfo[recipient_];
        if (info.gons > 0 && epoch.number >= info.expiry) {
            uint256 amount = ISSDOGE(sSDOGE).balanceForGons(info.gons);
    ...
    /**
        @notice forfeit sSDOGE in escrow and retrieve sdoge
     */
    function forfeit() external lock {
        ...
        if (info.gons > 0) {
            ...
            IERC20(sdoge).safeTransfer(msg.sender, info.deposit);
    ...
    function redeemWithPermit(
        ...
        IERC2612Permit(sSDOGE).permit(msg.sender, address(this), amount_, expiry, v, r, s);
    ...
    function _redeem(uint256 amount_, bool trigger_) internal {
        ...
        IERC20(sSDOGE).safeTransferFrom(msg.sender, address(this), amount_);
        IERC20(sdoge).safeTransfer(msg.sender, amount_);
    ...
    function rebase() public {
        if (epoch.nextBlock <= block.number) {
            ...
            uint256 staked = ISSDOGE(sSDOGE).circulatingSupply();
            ...
            ISSDOGE(sSDOGE).rebase(prevRewards, prevEpoch);
    ...
    function contractBalance() public view returns (uint256) {
        return IERC20(sdoge).balanceOf(address(this)).add(totalBonus);
    }
    ...
    function giveLockBonus(uint256 amount_) external {
        ...
        IERC20(sSDOGE).safeTransfer(locker, amount_);
    }
    ...
    function returnLockBonus(uint256 amount_) external {
        ...
        IERC20(sSDOGE).safeTransferFrom(locker, address(this), amount_);
    }
