...
import "./interfaces/ISDOGEBond.sol";
...
contract Reservoir is IReservoir, Ownable {
    ...
    uint256 public sSDOGEQueue; // Delays change to sSDOGE address
    ...
    constructor(
        ...
        require(sdoge_ != address(0), "SDOGE undefined");
        sdoge = sdoge_;
    ...
    {
        (uint256 value, address token) = ISDOGEBond(msg.sender).sdogeValue(amount_);
    ...
    function deposit(
        ...
        uint256 value = _sdogeValueOf(token_, amount_);
    ...
    function _deposit(
        ...
        IERC20Mintable(sdoge).mint(msg.sender, send_);
    ...
    /**
        @notice allow approved address to burn sdoge for collateral tokens
        @param bond_ address
        @param amount_ uint
     */
    function withdraw(address bond_, uint256 amount_) external isSpender() validate(bond_) {
        (uint256 value, address token) = ISDOGEBond(bond_).sdogeValue(amount_);
        require(_listContains(collateralTokens, token), "Collateral token only");

        ISDOGE(sdoge).burnFrom(msg.sender, value);
    ...
    function incurDebt(address bond_, uint256 amount_) external isDebtor() validate(bond_) {
        require(sSDOGE != address(0), "sSDOGE undefined");

        (uint256 value, address token) = ISDOGEBond(bond_).sdogeValue(amount_);
        require(_listContains(collateralTokens, token), "Collateral token only");

        uint256 maximumDebt = IERC20(sSDOGE).balanceOf(msg.sender); // Can only borrow against sSDOGE held
    ...
    /**
        @notice allow approved address to repay borrowed collaterals with sdoge
        @param amount_ uint
     */
    function repayDebtWithSDOGE(uint256 amount_) external isDebtor() {
        ISDOGE(sdoge).burnFrom(msg.sender, amount_);

        debtorBalances[msg.sender] = debtorBalances[msg.sender].sub(amount_);
        totalDebt = totalDebt.sub(amount_);

        emit LogRepayDebt(msg.sender, sdoge, amount_, amount_);
    }
    ...
    function manage(address bond_, uint256 amount_) external validate(bond_) {
        if (ISDOGEBond(bond_).isLiquidityBond()) {
    ...
    function mintRewards(address recipient_, uint256 amount_) external override isRewardManager() returns (uint256) {
        ...
        IERC20Mintable(sdoge).mint(recipient_, amount_);
    ...
    function excessAssets() public view returns (uint256) {
        uint256 net = IERC20(sdoge).totalSupply().sub(totalDebt);
    ...
    function auditAssets() external onlyOwner() {
        ...
        for (uint256 i = 0; i < bonds.length; i++) {
            ISDOGEBond bond = ISDOGEBond(bonds[i]);
            (value, ) = bond.sdogeValue(IERC20(bond.tokenIn()).balanceOf(address(this)));
    ...
    function queue(TARGETS target_, address address_) external onlyOwner() returns (bool) {
        ...
        } else if (target_ == TARGETS.S_SDOGE) {
            // 8
            sSDOGEQueue = block.number.add(queueLength);
        } else return false;
     ...
     */
    function _sdogeValueOf(address token_, uint256 amount_) internal view returns (uint256 value_) {
        value_ = amount_.mul(10**IERC20(sdoge).decimals()).div(10**IERC20(token_).decimals());
    }
}
