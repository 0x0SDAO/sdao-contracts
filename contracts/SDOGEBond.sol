...
contract SDOGEBond is Ownable {
    ...
    address public immutable reservoir; // mints sdoge when receives principle
    ...
    struct Bond {
        uint256 payout; // sdoge remaining to be paid
    ...
    function deposit(
        ...
        require(vars.payout >= 10000000, "Bond too small"); // must be > 0.01 sdoge ( underflow protection )
        ...
        uint256 sdogeMinted = IReservoir(reservoir).bondDeposit(amount_, vars.profit);

        require(sdogeMinted >= vars.value.sub(vars.profit), "Deposit failed");

        if (vars.fee > 0) {
            // fee is transferred to treasury
            IERC20(sdoge).safeTransfer(treasury, vars.fee);
        }

        if (vars.referrerRewards > 0) {
            IERC20(sdoge).safeIncreaseAllowance(referral, vars.referrerRewards);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(sdoge).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    function bondPriceInUSD() public view returns (uint256 price_) {
        if (isLiquidityBond) {
            price_ = bondPrice().mul(IBondCalculator(bondCalculator).markdown(tokenIn, sdoge)).div(100);
        } else {
     ...
     *   @return value_ uint
     */
    function sdogeValue(uint256 amount_) public view returns (uint256 value_, address token_) {
        if (isLiquidityBond) {
            value_ = IBondCalculator(bondCalculator).valuation(tokenIn, amount_);
        } else {
            // convert amount to match sdoge decimals
            value_ = amount_.mul(10**IERC20(sdoge).decimals()).div(10**IERC20(tokenIn).decimals());
        }
        token_ = tokenIn;
    }
    ...
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(sdoge).totalSupply();
    ...
    function standardizedDebtRatio() external view returns (uint256) {
        if (isLiquidityBond) {
            return debtRatio().mul(IBondCalculator(bondCalculator).markdown(tokenIn, sdoge)).div(1e9);
        } else {
            return debtRatio();
        }
    }
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding sdoge or principle token) to the treasury
     *  @return bool
     */
    function recoverLostToken(address token_) external returns (bool) {
        require(token_ != sdoge);
