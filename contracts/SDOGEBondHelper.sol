...
contract SDOGEBondHelper is Ownable {
    ...
    function deposit(
        ...
        vars.lpToken = ISDOGEBond(bond).tokenIn();
        require(vars.lpToken != address(0), "Bond principle undefined");

        address token0 = IPancakePair(vars.lpToken).token0();
        address token1 = IPancakePair(vars.lpToken).token1();
        ...
        ISDOGEBond(bond).deposit(liquidity, maxPrice_, recipient_, referralCode_);
