...
contract SDOGEBondPCSHelper is Ownable {
    ...
    function deposit(
        ...
        vars.lpToken = ISDOGEBond(vars.bond).tokenIn();
        require(vars.lpToken != address(0), "Bond principle undefined");

        vars.token0 = IPancakePair(vars.lpToken).token0();
        vars.token1 = IPancakePair(vars.lpToken).token1();
    ...
    function deposit(
        ...
        IPancakeRouter02(router).swapExactTokensForTokens(amountSwap, 0, path_, address(this), vars.deadline);
    ...
    function _depositLiquidity(address recipient_, DepositVars memory vars_) internal {
        ...
        ISDOGEBond(vars_.bond).deposit(liquidity, vars_.maxPrice, recipient_, vars_.referralCode);
    }
